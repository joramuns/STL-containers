...
TEST(Map, Constructor_Move) {
  ...
  auto it1_end = s21_map_2.end();
  while (it1 != it1_end) {
    EXPECT_EQ(it1.GetKey(), it2->first);
    EXPECT_EQ(*it1, it2->second);
    ++it1, ++it2;
  }
}
...
TEST(Map, Modifier_Insert) {
  ...
  auto it2 = s21_map_2.begin();
  while (it1 != s21_map_1.end()) {
    EXPECT_EQ(it1.GetKey(), it2.GetKey());
    EXPECT_EQ(*it1, *it2);
    ++it1, ++it2;
  }
  EXPECT_EQ(s21_map_1.size(), s21_map_2.size());
...
TEST(Map, Modifier_Erase_2) {
  s21::map<int, std::string> s21_map{
      {1, "aboba"}, {2, "shleppa"}, {3, "amogus"}, {4, "abobus"}};
  /* Disputable situation: after erasing by iterator, the iterator invalidates
   * and we cannot use ++ operator with it */
