...
TEST_F(ArrayTest, copy_constructor) {
  ...
  for (std::size_t i = 0; i < arr0_.size(); ++i) ASSERT_EQ(arr0_[i], arr1[i]);
}
...
TEST_F(ArrayTest, swap_string) {
  ...
  for (std::size_t i = 0; i < arr2_.size(); ++i) ASSERT_EQ(arr2_[i], arr2[i]);
}
...
TEST_F(ArrayTest, empty) { ASSERT_FALSE(arr0_.empty()); }
